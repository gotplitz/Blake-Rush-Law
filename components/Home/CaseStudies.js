    ...
    useEffect(() => {
        async function GetData() {
            const casesdata = await axios.get(
                'https://blakewp.ferociousmediaweb.com/wp-json/wp/v2/cases'
            );
            ...
            Promise.all([casesdata, titledata]).then((res) => {
                setCases(res[0].data);
                setTitlehome({
                    subtitle: res[1].data.fm_metabox.feature_two_h,
